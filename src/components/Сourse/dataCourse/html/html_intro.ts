...
export const html_intro2 = {
  ...
  HTML прошёл длинный путь с момента своего появления в начале 1990-х годов. Он превратился из простого языка с базовыми возможностями в полноценный стандарт для создания семантической, мультимедийной и интерактивной структуры веб-приложений.
  ...
  **Ключевые нововведения:**
  ...
  После HTML5 было принято решение больше не выпускать "номерных" версий (HTML6 и т.д.). Вместо этого стандарт развивается как **Living Standard**.
  
  - Поддерживается и развивается организацией **WHATWG**
  - Постоянно обновляется: новые возможности добавляются без выпуска новых мажорных версий
  - Браузеры синхронизируют поддержку по мере обновления спецификации
...
export const html_intro3 = {
  ...
  **Пример:**
  ...
  - HTML создаёт кнопку
  - CSS задаёт ей стиль: цвет, скругления, отступы, тень
  ...
  **Пример:**
  ...
  - HTML содержит форму
  - JavaScript проверяет данные и отправляет их на сервер, не перезагружая страницу
  ...
  Знание их взаимодействия даёт фундамент для изучения фреймворков (React, Vue, Angular), библиотек (Tailwind, Redux) и подходов к архитектуре (BEM, FSD, Atomic Design).
  `.trim(),
