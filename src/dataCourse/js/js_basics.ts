export const js_basics1 = {
  ...
  type: "js",
  content: `
  В JavaScript **операторы** — это специальные символы или ключевые слова, которые выполняют операции над значениями (переменными, выражениями и т.д.).
...
Логические операторы используются для объединения и проверки условий. Возвращают логические значения (true / false) или одно из значений выражений.
...
Работает по принципу **короткого замыкания** — если первое значение false, второе не вычисляется.
...
console.log(greeting); // "" — пустая строка, не выводит приветствие
\`\`\`

### \`||\` — логическое ИЛИ

Возвращает **первое истинное значение** или **последнее ложное**, если все выражения ложны.  
Также использует короткое замыкание — если первое значение true, второе не вычисляется.
...
let age = 25;
let isAdmin = false;
let access = (age >= 18 && age <= 65) || isAdmin;
