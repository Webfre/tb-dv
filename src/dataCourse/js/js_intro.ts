...
export const js_intro3 = {
  ...
  type: "js",
  content: `
### Как работает JavaScript? Основы для начинающих

JavaScript — это язык программирования, который позволяет создавать интерактивные и динамические элементы на веб-страницах. Он часто используется для обработки событий, работы с формами, выполнения запросов к серверу и управления контентом страницы.

---

#### Что такое JavaScript?
JavaScript — это интерпретируемый язык, который выполняется в браузере или на сервере. Его главная задача — добавлять интерактивность на сайт.

Основные характеристики JavaScript:
1. **Динамичность** — позволяет менять контент страницы без перезагрузки.
2. **Однопоточность** — код выполняется последовательно, но поддерживает асинхронные операции.
3. **Событийно-ориентированный** — реагирует на действия пользователя, например, нажатие кнопки.
4. **Кроссплатформенность** — работает в браузерах, на сервере (Node.js) и в мобильных приложениях (React Native).

---

#### Как исполняется код?
Браузер интерпретирует код построчно, начиная с самого верхнего файла скрипта и заканчивая последним. JavaScript не компилируется заранее, как C++ или Java. 

##### Пример:
\`\`\`js
let x = 5;     // Создаем переменную
x = x + 2;     // Изменяем её значение
alert(x);      // Показываем результат: 7
\`\`\`
...
- **В мобильных приложениях**: например, через React Native

---

#### Основные компоненты движка JavaScript:
1. **Call Stack (стек вызовов)** — хранит вызванные функции и обрабатывает их поочерёдно.
2. **Heap (куча)** — хранит объекты и данные, которые требуют долгого хранения.
3. **Web APIs** — встроенные возможности браузера (например, таймеры, DOM).
4. **Callback Queue** — очередь обратных вызовов, которые ожидают выполнения.
...
#### Работа с DOM
JavaScript может динамически изменять содержимое страницы, например, при нажатии на кнопку:
\`\`\`html
...
JavaScript работает в одном потоке, чтобы избежать конфликтов при доступе к DOM. Однако благодаря асинхронным операциям (например, через колбэки, промисы и async/await) он может выполнять долгие задачи без блокировки основного потока.

---

#### Заключение
JavaScript — мощный инструмент для создания интерактивных и динамичных веб-приложений. Его способность работать асинхронно позволяет обрабатывать действия пользователя и данные с сервера без задержек.

Рекомендуется изучить такие темы, как асинхронные функции, работа с DOM, управление событиями и основы объектно-ориентированного программирования на JavaScript.
`,
  codeExample: `
