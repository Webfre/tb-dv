...
export const html_pr_1 = {
  id: "html_pr_1",
  title: "Практическая работа: сверстать семантическую веб-страницу",
  moduleId: "html",
  content: `
...
- Построить **семантическую структуру** документа: в шапке (\`<header>\`) разместите название проекта и основную навигацию (\`<nav>\`); основной контент (\`<main>\`) разделить на тематические секции (\`<section>\`), внутри которых будут отдельные статьи (\`<article>\`) и при необходимости боковая панель (\`<aside>\`); внизу разместите подвал (\`<footer>\`) с копирайтом и дополнительными ссылками.
- Продемонстрировать навыки работы с **заголовками** (\`<h1>\`–\`<h4>\`), **абзацами** (\`<p>\`), **списками** (\`<ul>\`, \`<ol>\`, \`<li>\`), **таблицами** (\`<table>\` с \`<caption>\`, \`<thead>\`, \`<tbody>\`, \`<tfoot>\`) и **формами** (\`<form>\` с текстовыми полями, выпадающими списками, радио-кнопками, чекбоксами и кнопкой отправки).
- Включить в \<head\> обязательные **мета-теги** для кодировки, адаптивности и описания страницы, а также зафиксируйте заголовок \<title\>.
- Обеспечить **доступность**: используйте понятные подписи для полей форм, атрибуты \`alt\` и \`title\` у изображений, корректную иерархию заголовков и семантические теги.
