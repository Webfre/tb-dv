...
export const js_intro_task_2 = {
  ...
  steps: [
    ...
    "Объявите переменную с помощью `let` и присвойте ей строку. Измените значение и выведите оба раза через `console.log()`.",
    "Объявите переменную с помощью `const` и присвойте ей число. Попробуйте изменить значение и проверьте, что произойдёт (будет ошибка).",
    "Объявите переменную с помощью `var`, присвойте ей строку. Выведите её значение до и после присваивания (до объявления).",
    "Посмотрите на поведение каждой переменной в консоли, чтобы понять разницу между `let`, `const` и `var` (область видимости, возможность переопределения, всплытие).",
  ],
  difficulty: 1,
  solution: `<!DOCTYPE html>
<html lang="ru">
